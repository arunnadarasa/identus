...
type Session = {
  program: unknown;
  noir: { execute: (inputs: Record<string, number>) => Promise<{ witness: Uint8Array }> };
  backend: {
...
export default function ZkProofLive() {
  ...
  const wouldPass = useMemo(() => dobValid && dobNumber <= thresholdYear, [dobValid, dobNumber, thresholdYear]);
  ...
  return (
    <div className="rounded-lg border border-primary/30 bg-card/40 p-4 sm:p-6">
      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
        <div className="min-w-0">
          <h3 className="font-mono text-sm font-medium text-primary">
            Prove I'm over 18 — for real
          </h3>
          <p className="mt-1 text-sm text-muted-foreground">
            This runs an actual zero-knowledge proof in your browser using{" "}
            <a
            ...
            and the Barretenberg UltraHonk prover. Nothing is sent to a server.
          </p>
        </div>
      ...
      <p className="mt-2 text-xs text-muted-foreground">
        <span className="text-foreground/80">dob_year</span> is private — it never leaves the prover.{" "}
        <span className="text-foreground/80">threshold_year</span> is public, so the verifier can read
        it back from the proof.
      </p>

      <div className="mt-5 grid gap-4 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
        <div className="space-y-2">
          <Label htmlFor="zk-dob" className="text-xs">
            Your birth year (private input)
          </Label>
          <Input
            id="zk-dob"
            inputMode="numeric"
      ...
      <div className="mt-5 flex flex-col gap-2 sm:flex-row">
        <Button onClick={handleProve} disabled={busy || !dobValid} className="w-full sm:w-auto">
          {busy ? (
            <>
